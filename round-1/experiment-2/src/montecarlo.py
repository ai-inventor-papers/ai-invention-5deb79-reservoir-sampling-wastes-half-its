...
from nullband import (
    PairAccumulator,
    analytic_maxdev,
    coinclusion_stats,
    first_order_stats,
)
from prngs import make_prng
...
def run_bits(
    ...
    *,
    ...
    for sd in seeds:
        ...
        del src, res
    a = budget.accept_H(n, k)
    e = budget.evict_H(n, k)
    ...
    return {
        ...
        "closed_form_naive64": budget.naive_total(n, k, 64),
        "closed_form_ky": budget.ky_accept(n, k) + e,
...
def bernoulli_variant_costs(
    ...
    Knuth-Yao DDG walk drawing its bits from the same buffer, which costs 2
    buffer-bits per call regardless of p.  Both are exact; the plan asked for the
    cheaper one to be reported rather than assumed, so both are measured here.
    """
    import math

    from bits import make_source
