...
see the parent 'S1' candidate in candidates.py).  This module supplies the
...
resolves only the U = Ns - Nt*d leftover sources with a MINIMAL, exact,
...
# --------------------------------------------------------------------------- #
#  3. s1t_children -- the exact-verifier / sampler transition kernel          #
# --------------------------------------------------------------------------- #
...
# --------------------------------------------------------------------------- #
#  4. verify_s1t -- exact rational forward propagation (the acceptance test)  #
# --------------------------------------------------------------------------- #
def verify_s1t(n: int, k: int) -> dict:
    """Exact rational prefix verification of S1t, mirroring exact.verify's loop
    shape but driven locally (candidates.py / exact.py are not touched)."""
    cand = Candidate(
        "S1t", "SUMMODK+MINFLOW", scalar=None, vector=None, children=s1t_children, family="s1t",
...
def extrapolate_total(
    ...
    """Extrapolated S1t total eviction bits over a stream of length n, plus the
...
def main() -> None:
    ...
    logger.info("(b) verify_s1t exact-uniformity acceptance test, k in {2,3,4} at n=12")
    output["verify_s1t"] = {}
    for k in (2, 3, 4):
    ...
    for k in (10, 100):
        ...
        for n in (1_000_000, 10_000_000):
            ...
            logger.info(
                ...
                f"exact_partial_sum(i<={res['exact']['max_i_solved']})="
                f"{res['exact']['partial_sum_evict_bits_over_solved_i']:.6f}"
            )
