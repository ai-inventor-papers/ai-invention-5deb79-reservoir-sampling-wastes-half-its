"""algol.py -- float64 bias audit of Algorithm L's geometric-skip recurrence.
...
--------------------------------------------------------------------------
The common claim that `1.0 - W` suffers catastrophic cancellation is WRONG.
...
`notes["w_to_zero_correction"]` for the exact numbers from the run that
produced this file.
...
(same law), this is a legitimate paired reuse of the same draw budget, not
...
its OWN mpmath dps=80 exact counterpart (`skip_exact(..., kind=...)`),
computed from an exact, lossless float64->mpf conversion of the very same
...
def skip_shipped(W: float, u2: float) -> float:
    ...
    an exact zero denominator, and `math.floor` raises OverflowError on an
...
def skip_exact(state: float, u2: float, kind: str, dps: int = 80) -> float:
    """mpmath dps=80 exact reference skip, mirroring `skip_shipped`/`skip_fixed`/`skip_logdomain`.

    `state` and `u2` are float64 values; they convert to `mpf` losslessly
    (every finite float64 is an exact dyadic rational), so no additional
...
# --------------------------------------------------------------------------- #
# 2. State-advance recurrences (numpy-vectorized float64 paths, mpmath exact path)
# --------------------------------------------------------------------------- #
...
def _advance_exact(U_rows: np.ndarray, k: int, kind: str, dps: int) -> list[Any]:
    """mpmath dps=80 exact final state for each row of U_rows, shape (n_exact, n_steps).
    ...
    (~266 bits) the two are indistinguishable to far more digits than are
    ...
    keeps the mpmath path fast enough to honor the runtime budget.

        shipped:    ln(W)   = sum_j ln(U_j)     / k   ->  W    = exp(...)
        fixed:      ln(1-V) = sum_j log1p(-U_j) / k   ->  V    = -expm1(...)
        logdomain:  logW    = sum_j ln(U_j)     / k   ->  logW = ... (kept as-is)

    Every float64 U_j converts to `mpf` losslessly (exact dyadic value), so
...
def audit_algo_l(k: int, n_draws: int, seed: int, position_steps: int, dps: int = 80) -> dict[str, Any]:
    ...
    across all `n_draws` trajectories; only the mpmath exact reference is
    computed with a python-level loop, and only over a (possibly
    subsampled) subset of trajectories, per the runtime budget below.

    Returns a dict of per-cell metrics; see `run_algol_audit`'s returned
    `notes["metric_definitions"]` for the exact definition of every field.
    """
    ...
    # --- mpmath dps=80 exact reference, subsampled for speed ------------- #
    # Spec requires: if n_draws > 20000, subsample 20000 pairs for the
    # exact reference. We add a second, stricter adaptive cap so that the
    ...
    # budget, since the spec's own cap alone does not bound the
...
def audit_realistic_regime(k: int, n_list: list[float], n_draws: int, seed: int, dps: int = 80) -> list[dict[str, Any]]:
    ...
    exact mpmath dps=80 reference AT THAT EXACT TARGET, for n spanning
    ordinary to extreme stream lengths. Because the state is a single fixed
    injected value (not a per-trial trajectory), only ONE exact rate
    reference per n is needed regardless of n_draws, so this stays fast
    even at large n_draws; only the per-trial exact SKIP (which does vary
    with u2) needs one mpmath floor/log per draw, still O(1) work per draw
    (no trajectory loop), unlike `audit_algo_l`'s exact path.
    """
    ...
    for n in n_list:
        ...
        with mp.workdps(dps):
            W_exact = mp.mpf(k) / mp.mpf(n)  # exact target ratio, no float64 rounding at all
            V_exact = 1 - W_exact
...
def run_algol_audit(
    ...
    w_to_zero_correction = (
        "BLUNT CORRECTION: the common 'catastrophic cancellation in 1-W' story is WRONG. "
    ...
    notes = {
        "sterbenz_correction_original_and_wrong": (
            ...
            "corrected diagnosis] The common story that '1.0 - W' suffers catastrophic "
        ...
        "metric_definitions": {
            "disagree_rate_f64 / disagree_rate_fixed / disagree_rate_logdomain": "P(skip_variant != skip_exact(kind=variant)) over the (possibly subsampled) exact-reference pairs, one probability per float64 variant against its OWN mpmath dps=80 exact counterpart.",
            ...
            "fix_improvement_factor / fix_improvement_factor_logdomain": "disagree_rate_f64 / max(disagree_rate_{fixed,logdomain}, 1/n_draws) -- how many times more often the shipped path disagrees with its exact reference than the fixed/logdomain path disagrees with its own.",
        ...
        "paired_draw_convention": (
            ...
            "OWN mpmath dps=80 exact counterpart, never against another variant's exact "
