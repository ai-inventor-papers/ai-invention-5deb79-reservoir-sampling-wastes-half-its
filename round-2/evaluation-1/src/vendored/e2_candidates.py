...
vectorised (Monte-Carlo) implementation, plus the e2_exact transition kernel used
by the rational verifier in :mod:`e2_exact`.
...
S1n    SUMMODK-NOREPAIR     same, never randomised -- negative control AND the
                            zero-eviction-e2_bits upper bound on savings
...
S5g    PAIR-LOCK            classical (k/2)-reservoir over consecutive PAIRS:
                            e2_exact first-order, support C(n/2, k/2)
...
# --------------------------------------------------------------------------- #
#  e2_exact transition kernels (consumed by e2_exact.py)                             #
# --------------------------------------------------------------------------- #
...
# --------------------------------------------------------------------------- #
class Candidate:
    """One sampler: scalar bit-counted, vectorised, and e2_exact-kernel views."""
...
CANDIDATES: dict[str, Candidate] = {
    ...
    "S1n": Candidate(
        ...
        family="control", note="Negative control and the zero-eviction-e2_bits bound.",
    ),
    "S2": Candidate(
        "S2", "ALGO-L-float64", scalar=s2_scalar, vector=s2_vec,
        children=None, family="e2_algol",
    ...
    "S5g": Candidate(
        ...
        note="Classical (k/2)-reservoir over consecutive PAIRS: e2_exact "
    ...
    "S6sys": Candidate(
        ...
        note="Textbook systematic sampling with a random start. Needs n, so it "
             ...
             "first-order-e2_exact / joint-degenerate exhibit (support n/k).",
    ),
}
