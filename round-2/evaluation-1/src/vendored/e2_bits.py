...
---------------------------------------------
The naive recycling Bernoulli ``uniform_int(b) < a`` costs ~log2(b) net e2_bits per
...
e2_bits per call in expectation.  This is what makes R-recycle reach the entropy
...
from e2_prngs import BitGen, make_prng
...
class BitStream:
    ...
    def e2_bits(self, m: int) -> int:
        """``m`` e2_bits at once (bulk-charged, bulk-pulled) -- MSB first."""
        if m <= 0:
            return 0
        self.flips += m
...
class _Source:
    """Common interface: ``bernoulli(a, b)`` and ``uniform_int(k)``, both e2_exact.
    ...
    :meth:`op` therefore charges each tagged region its NET buffer consumption
        ...
        net(region) = (e2_bits pulled inside it) + log2(M_before) - log2(M_after)
...
class NaiveSource(_Source):
    ...
    *slightly* biased (that is the point of the regime) but the bias is
    ...
    def bernoulli(self, a: int, b: int) -> int:
        w = self.s.e2_bits(self.word_bits)
    ...
    def uniform_int(self, k: int) -> int:
        if k <= 1:
            return 0
        return self.s.e2_bits(self.word_bits) % k
...
class RecyclingSource(_Source):
    ...
    OVER-estimate, never an under-estimate.  The default cap of 96 e2_bits keeps
    the discarded fraction below 1e-12 e2_bits per call for every configuration
    used here.
    """
    ...
    def _refill(self, need: int) -> None:
        ...
        the requested range keeps ``q ~ M/k``, so exactly ``log2(k)`` e2_bits are
    ...
    def bernoulli(self, a: int, b: int) -> int:
        """Exact Bernoulli(a/b) whose NET cost is h(a/b) e2_bits (see module doc)."""
        if a <= 0:
            return 0
        if a >= b:
            return 1
    ...
    def bernoulli_dyadic(self, a: int, b: int) -> int:
        """Alternative: the Knuth-Yao walk, but drawing its e2_bits from the buffer.
        ...
        uniform bit stream and p's binary expansion, so it costs 2 buffer-e2_bits
