#!/usr/bin/env python3
"""Is the accept coin e1_forced?  And what does anytime uniformity cost in fair coin flips?
...
its solution set is the single point p == k/(i+1): the accept probability is e1_forced and
...
PART 2 -- BIT BUDGETS.  The e1_forced coin costs at least sum_i h(k/i) fair bits
(Shannon/Knuth-Yao).  The classical rule additionally spends log2(k) bits per acceptance.
The pre-registered analytic bracket from e1_fibermap says the MINIMUM eviction cost per
...
from e1_fibermap import binary_entropy
