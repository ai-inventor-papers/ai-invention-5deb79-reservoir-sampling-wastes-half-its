...
Propagates the e2_exact distribution over reservoir states with
...
from e2_candidates import Candidate, get_candidate
...
def verify(cid: str, n: int, k: int, *, record_every: bool = False) -> dict:
    ...
    if cand.children is None:
        raise ValueError(f"candidate {cid} has no e2_exact kernel (audited elsewhere)")
