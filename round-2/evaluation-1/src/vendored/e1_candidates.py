...
Everything here is measured by the SAME exact DP (``e1_exact_dp.run_exact_dp``), so no
...
class SumModK(_SetSampler):
    ...
    def evict(self, slots, i, k):
        return [(e1_fibermap.summodk_position(slots, k), Fraction(1))]
...
class TableDriven(_SetSampler):
    ...
    def _table(self, i: int, k: int):
        ...
        if i not in self._tables:
            self._tables[i] = e1_fibermap.eviction_table(self.rule, i - 1, k)
