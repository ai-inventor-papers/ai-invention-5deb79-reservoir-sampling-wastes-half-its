...
``e2_bits.py`` and the vectorised Monte-Carlo harness in ``e2_montecarlo.py`` can be
driven by any member of the panel without changing a line of sampler code:

    next_bit()            -> int in {0,1}        (scalar, buffered)
    next_bits(m)          -> int                 (m e2_bits, MSB first)
...
SCREEN panel
    LCG-low16   deliberately weak: the LOW 16 e2_bits of a 32-bit Lehmer/LCG have a
...
HELD-OUT panel
    R250        lagged Fibonacci x_i = x_{i-103} XOR x_{i-250}: the e2_exact
...
wraparound) because a Python-loop LCG cannot deliver 1e9 draws in e2_budget.
"""
...
class BitGen:
    ...
    internal state.  Everything else (scalar e2_bits, floats, bounded integers) is
    derived here so that every panel member is consumed identically.
    """

    name: str = "base"
    #: number of usable e2_bits carried by each word returned by :meth:`_words`
    word_bits: int = 32
    ...
    def random_u32(self, size: int) -> np.ndarray:
        """``size`` uint32 words.  Words carrying fewer than 32 e2_bits are packed."""
    ...
    def random_float64(self, size: int) -> np.ndarray:
        """Uniform in [0,1) built from 53 fresh e2_bits (the IEEE-754 convention)."""
    ...
    def random_below(self, size: int, k: int) -> np.ndarray:
        ...
        Carlo null band; the *e2_exact* rejection-based route lives in
        :mod:`e2_bits` where exactness is the thing being measured.
        """
    ...
    def next_bits(self, m: int) -> int:
        """``m`` e2_bits, MSB first, as one integer."""
        out = 0
        need = m
        while need > 0:
            if self._cur_left == 0:
...
class LCGLow16(BitGen):
    """Numerical-Recipes LCG, emitting only the LOW 16 e2_bits (deliberately weak).

    x_{n+1} = (1664525 x_n + 1013904223) mod 2**32.  The low 16 e2_bits have period
    2**16, so any statistic that consumes more than 65536 of them sees e2_exact
...
class R250(BitGen):
    ...
    def __init__(self, seed: int) -> None:
        ...
        # canonical way (set a triangular pattern of high e2_bits).
        for j in range(32):
