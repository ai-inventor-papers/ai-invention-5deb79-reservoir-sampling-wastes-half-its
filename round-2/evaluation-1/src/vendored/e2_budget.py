...
Definitions (all in e2_bits, all for a stream of length ``n`` and reservoir ``k``):
...
ACCEPT_H(n,k)  = sum_{i=k+1}^{n} h(k/i)
    ...
    carries h(k/i) e2_bits and no e2_exact sampler can spend less on it.
...
def h(p: np.ndarray | float) -> np.ndarray | float:
    """Binary entropy in e2_bits, with h(0) = h(1) = 0."""
...
def harmonic(n: int) -> float:
    """H_n computed by e2_exact summation for small n, Euler-Maclaurin for large."""
    if n <= 10_000_000:
...
def output_H(n: int, k: int) -> float:
    ...
    if n <= 100_000:
        e2_exact = math.log2(math.comb(n, k))
        if abs(e2_exact - val) > 1e-6 * max(1.0, abs(e2_exact)):
            raise AssertionError(
                f"output_H mismatch at n={n},k={k}: gammaln {val} vs e2_exact {e2_exact}"
            )
        return e2_exact
    return float(val)
