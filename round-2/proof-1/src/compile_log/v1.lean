...
theorem uniformPlan_row {i k : ℕ} (hk1 : 1 ≤ k) (hki : k ≤ i)
  ...
  unfold uniformPlan
  rw [Finset.sum_ite_mem] at *
  rw [Finset.sum_ite, Finset.sum_const, Finset.sum_const_zero, add_zero, hcard, nsmul_eq_mul]
...
theorem uniformPlan_col {i k : ℕ} (hk1 : 1 ≤ k) (hki : k ≤ i)
  ...
  have hcard := Finset.card_filter_powersetCard_subset T (Finset.range i) k hTsub hTk
  rw [hTcard, Finset.card_range] at hcard
  have e1 : i - (k - 1) = i - k + 1 := by omega
  have e2 : k - (k - 1) = 1 := by omega
  rw [e1, e2, Nat.choose_one_right] at hcard
  unfold uniformPlan
  rw [Finset.sum_ite, Finset.sum_const, Finset.sum_const_zero, add_zero]
  have hfe : (Finset.powersetCard k (Finset.range i)).filter (fun S => T ⊆ S)
      = {x ∈ Finset.powersetCard k (Finset.range i) | T ⊆ x} := rfl
  rw [hfe, hcard, nsmul_eq_mul]
  have hkR : (0:ℝ) < (k:ℝ) := by exact_mod_cast hk1
  have : ((i - k + 1 : ℕ) : ℝ) = (i:ℝ) - (k:ℝ) + 1 := by
    have : ((i - k : ℕ) : ℝ) = (i:ℝ) - (k:ℝ) := by
      rw [Nat.cast_sub hki]
    push_cast [this]
    ring
  rw [this]
...
theorem forest_split_count {A : ℕ} (d : Fin A → ℕ) (hd : ∀ j, 1 ≤ d j)
  ...
  classical
  set s : Finset (Fin A) := Finset.univ.filter (fun j => 2 ≤ d j) with hs
  set t : Finset (Fin A) := Finset.univ.filter (fun j => ¬ (2 ≤ d j)) with ht
  have hsplit : (∑ j ∈ s, d j) + (∑ j ∈ t, d j) = ∑ j, d j :=
    Finset.sum_filter_add_sum_filter_not Finset.univ (fun j => 2 ≤ d j) d
  have h1 : 2 * s.card ≤ ∑ j ∈ s, d j := by
    have : ∑ _j ∈ s, 2 ≤ ∑ j ∈ s, d j :=
      Finset.sum_le_sum (fun j hj => by
        have := (Finset.mem_filter.mp hj).2
        omega)
    simpa [Finset.sum_const, smul_eq_mul, mul_comm] using this
  have h2 : t.card ≤ ∑ j ∈ t, d j := by
    have : ∑ _j ∈ t, 1 ≤ ∑ j ∈ t, d j := Finset.sum_le_sum (fun j _ => hd j)
    simpa [Finset.sum_const, smul_eq_mul] using this
  have hcards : s.card + t.card = A := by
    have := Finset.filter_card_add_filter_neg_card_eq_card
      (s := (Finset.univ : Finset (Fin A))) (p := fun j => 2 ≤ d j)
    simpa [hs, ht, Finset.card_univ] using this
  omega
...
theorem entropy_le_log_card {α : Type*} [DecidableEq α] (s : Finset α) (q : α → ℝ)
  ...
  have hterm : ∀ a ∈ t, Real.negMulLog (q a) ≤ q a * Real.log (s.card) + 1 / (s.card:ℝ) - q a := by
    ...
    have hsimp : q a * (1 / ((s.card:ℝ) * q a) - 1) = 1 / (s.card:ℝ) - q a := by
      field_simp
    rw [hsimp] at hmul
    have : Real.negMulLog (q a) = -(q a * Real.log (q a)) := by
  ...
  have hrhs : ∑ a ∈ t, (q a * Real.log (s.card) + 1 / (s.card:ℝ) - q a)
    ...
    rw [Finset.sum_sub_distrib, Finset.sum_add_distrib, ← Finset.sum_mul, hsumt,
  ...
  rw [hrestr]
  linarith [hsumbound, hrhs.le, hrhs.ge]
...
theorem entropy_le_logb_card {α : Type*} [DecidableEq α] (s : Finset α) (q : α → ℝ)
  ...
  rw [H2, Real.logb]
  exact div_le_div_of_nonneg_right h hl2 |>.trans_eq rfl
...
theorem telescope_bound (m : ℕ) :
  ...
  have key : ∀ j : ℕ, (1:ℝ) / (((j:ℝ) + 1) * ((j:ℝ) + 2))
      = -((1:ℝ) / ((j:ℝ) + 1 + 1) - (1:ℝ) / ((j:ℝ) + 1)) := by
    intro j
    have h1 : ((j:ℝ) + 1) ≠ 0 := by positivity
    have h2 : ((j:ℝ) + 2) ≠ 0 := by positivity
    field_simp
    ring
  have hsum : ∑ j ∈ Finset.range m, (1:ℝ) / (((j:ℝ) + 1) * ((j:ℝ) + 2))
      = -(((1:ℝ) / ((m:ℝ) + 1)) - 1) := by
    have := Finset.sum_range_sub (fun j : ℕ => (1:ℝ) / ((j:ℝ) + 1)) m
    calc ∑ j ∈ Finset.range m, (1:ℝ) / (((j:ℝ) + 1) * ((j:ℝ) + 2))
        = ∑ j ∈ Finset.range m,
            -((1:ℝ) / (((j:ℝ) + 1) + 1) - (1:ℝ) / ((j:ℝ) + 1)) := by
          refine Finset.sum_congr rfl (fun j _ => ?_)
          rw [key j]
      _ = -(∑ j ∈ Finset.range m,
            ((fun j : ℕ => (1:ℝ) / ((j:ℝ) + 1)) (j + 1)
              - (fun j : ℕ => (1:ℝ) / ((j:ℝ) + 1)) j)) := by
          rw [← Finset.sum_neg_distrib]
          refine Finset.sum_congr rfl (fun j _ => ?_)
          push_cast
          ring_nf
      _ = -(((1:ℝ) / ((m:ℝ) + 1)) - 1) := by rw [this]; norm_num
  rw [hsum]
  have : (0:ℝ) < (1:ℝ) / ((m:ℝ) + 1) := by positivity
  linarith
...
theorem total_constant_in_n (k n : ℕ) (hk1 : 1 ≤ k) :
  ...
  nlinarith [h, hlogk, sq_nonneg ((k:ℝ))]
